...
void Camera::update(Protagonist &protagonist) {
  ...
  m_eye.y = protagonist.getPosition().y + vDistance;
  m_eye.x = protagonist.getPosition().x - offsetX;
  m_eye.z = protagonist.getPosition().z - offsetZ;

  m_at = protagonist.getPosition() + glm::vec3{0.0f, 0.5f, 0.0f};
  computeViewMatrix();
}
...
void Camera::zoom(float scroll) {
  if (scroll > 0 && m_protagonistDistance < 2.0f)
    return;
  if (scroll < 0 && m_protagonistDistance > 6.0f)
    return;
